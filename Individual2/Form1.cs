...
namespace Individual2
{
    public partial class Form1 : Form
    {
        ...
        Point3D positionLight2 = new Point3D(0, 9, -5);
        double min = 1.0;
        double max = 10.0;
        public Form1()
        {
        ...
        private void addCubes(Material matte, Material mirror, Material glass)
        {
            ...
            {
                cube = new Cube(1.0, new Point3D(4.0, 0.5, 0.0), Color.Red, matte); // Матовый куб
                scene.Add(cube);
            }
            if (mirrorCube.Checked)
            {
                cube = new Cube(1.0, new Point3D(-4.0, 1.5, 1.0), Color.White, mirror); // Зеркальный куб
                scene.Add(cube);
            }
            if (transparentCube.Checked)
            {
                cube = new Cube(1.0, new Point3D(2.0, 0, -5.0), Color.White, glass); //Прозрачный куб
                scene.Add(cube);
            }
        }
        ...
        private void addLights()
        {
            ...
            if (light2.Checked && (textBoxX.Text != "" || textBoxY.Text != "" || textBoxZ.Text != ""))
            {
                positionLight2 = new Point3D(float.Parse(textBoxX.Text), float.Parse(textBoxY.Text), float.Parse(textBoxZ.Text));   
